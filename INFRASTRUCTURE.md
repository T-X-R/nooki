...
## Conversation document editing

Conversations support UTF-8 Markdown (`.md`) and text (`.txt`) attachments, including documents selected from the Library. Each session uses its own directory under `conversation-workspaces/<thread hash>`. Nooki copies selected source snapshots and uploaded text into this directory. Reattaching the same Library document preserves the session's existing working copy, including edits from earlier turns. Original Library documents, source snapshots and uploaded files are not modified by generation.

Every turn explicitly selects `workspaceWrite` with only the session directory, no network access, no shared temporary-directory writes and no approval escalation. This is a filesystem write boundary, not a restriction to particular file extensions or a read-access sandbox. The document surface accepts regular UTF-8 `.md`/`.txt` files directly in the workspace: at most 50 files, 2 MB per file and 8 MB total. Other files are not exposed as document results. Library source bodies are accessed through working files instead of duplicated into the prompt. Source references remain available for citations.

The platform stores input baselines and completed document results in `conversation-document-state`, outside the agent's writable directory. A completed response returns changed/new files as durable task results. Earlier turn previews retain their content after follow-up edits and reconnection. Retry reuses an already retained result. If a turn's output was not retained and newer work has already run, recovery reports that boundary rather than attributing newer file content to the old turn. Cancelled or failed work is not automatically published; its working files remain available for a follow-up.

Document cards provide preview, changes compared with the beginning of that turn, download to the user's Downloads directory, and explicit saving. Saving reuses the platform publication job. The save dialog selects a Topic and a section and publishes a new Library document; section placement is independent of source identity. It does not display or edit the document body. Existing revision and conflict handling remains available for retained publication jobs. Ordinary conversational answers remain independently savable. Backups retain document state and supported working files, but still do not include Codex-owned conversation history or arbitrary executable workspace files.

Verification covers attachment validation, independent workspaces, unchanged originals, consecutive edits, immutable turn results, restart recovery, backup/restore and existing publication conflict behavior. A live two-turn check through the local Codex CLI edited synthetic Library and TXT inputs and verified that the first turn remained recoverable after the second; no user documents were used. Visual acceptance is left to the user.
