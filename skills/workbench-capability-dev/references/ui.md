...
Use the starter's `ui` modules for page layout, headings, panels, buttons and empty/error states. They follow Workbench colors and dimensions through host CSS variables. Custom UI can compose these primitives without importing platform pages or global classes.

- The capability occupies the content area. Workbench owns navigation, the window shell and global settings.
...
The `preview` folder is a development-only harness. Its theme file is derived from Workbench; do not import it into the package entry. Preview storage is namespaced by capability ID. Task checkpoints, AI, source grants and native lifecycle require desktop verification.
