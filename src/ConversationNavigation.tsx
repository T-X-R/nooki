...
import { CONVERSATION_OWNER, type Conversation } from './conversation-model'
...
export function ConversationNavigation({ language, active, selectedId, onEnter, onSelect }: {
  ...
  return <div className="nav-conversations">
    ...
    {expanded && <div className="nav-conversation-history" id="sidebar-conversation-history">
      ...
      {sessions.map((session) => <button key={session.id} title={session.name || session.preview} aria-current={active && selectedId === session.id ? 'page' : undefined} onClick={() => onSelect(session.id)}><span>{session.name || session.preview || (zh ? '新对话' : 'New conversation')}</span></button>)}
      ...
      {!busy && !error && !sessions.length && <p>{zh ? '还没有历史会话' : 'No conversations yet'}</p>}
