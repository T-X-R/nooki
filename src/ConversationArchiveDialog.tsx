...
import { ArchiveIcon, ResetIcon, TrashIcon } from '@radix-ui/react-icons'
import { conversationClient } from './conversation-client'
import type { Conversation } from './conversation-model'
import { LibraryDialog } from './LibraryDialogs'

export function ConversationArchiveDialog({ language, onClose, onRestore }: { language: 'zh' | 'en'; onClose(): void; onRestore(id: string): void }) {
  ...
  useEffect(() => {
    ...
    void readArchive(() => current).then((rows) => { if (current) setSessions(rows) })
  ...
  return <LibraryDialog className="conversation-archive-dialog" title={deleting ? (zh ? '永久删除会话' : 'Delete conversations permanently') : (zh ? '已归档会话' : 'Archived conversations')} busy={busy} onClose={onClose}>
    {deleting ? <>
