...
export function TaskPage({ language, installed, selectedId, onOpenCapability }: { selectedId?: string | null; onOpenCapability?: (id: string) => void; language: 'zh' | 'en'; installed: InstalledCapability[] }) {
  ...
  return <div className="content-column">
    <div className="page-header-row"><div><div className="eyebrow">TASKS</div><h1>{zh ? '任务' : 'Tasks'}</h1><p>{zh ? '查看能力的执行进度，从中断或失败处继续。' : 'Follow capability work and resume interrupted or failed tasks.'}</p></div></div>
    {error && <p role="alert">{error}</p>}
    {!records.length && <div className="surface activity-empty">{zh ? '暂无任务。从支持任务的能力中开始一次执行。' : 'No tasks yet. Start work from a capability.'}</div>}
