...
import { CheckIcon, ChevronDownIcon, Cross2Icon, DownloadIcon, PlusIcon, TrashIcon } from '@radix-ui/react-icons'
  ...
  return <>
    <div className="library-management-bar" onDragOver={(e) => { if (e.dataTransfer.types.includes('Files')) e.preventDefault() }} onDrop={(e) => { e.preventDefault(); loadFiles(Array.from(e.dataTransfer.files)) }}>
      ...
      <div className="library-management-group">
        ...
        <button className="quiet-button" disabled={busy} onClick={() => { setImports([{ key: crypto.randomUUID(), title: '', content: '', documentDate: new Date().toLocaleDateString('en-CA') }]); setActive(0); setPreview(false) }}>{zh ? '粘贴文字' : 'Paste text'}</button>
    ...
    {manageTopics && <LibraryDialog title={zh ? '管理专题' : 'Manage topics'} busy={busy} onClose={() => setManageTopics(false)}><p className="modal-copy">{zh ? '将不同来源的资料整理在一起。默认视图始终包含全部资料。' : 'Organize documents across sources. Default always contains every document.'}</p><div className="library-topic-list">{organization.topics.map((t) => <div key={t.id}><span><strong>{t.name}</strong><small>{t.documentIds.filter((id) => documents.some((d) => d.id === id)).length} {zh ? '篇资料' : 'documents'}</small></span><button className="quiet-button" disabled={busy} onClick={() => { setTopic({ ...t }); setError('') }}>{zh ? '改名' : 'Rename'}</button><button className="quiet-button library-delete-action" disabled={busy} onClick={() => { setTopic({ ...t }); setConfirm('topic'); setError('') }}>{zh ? '删除' : 'Delete'}</button></div>)}{!organization.topics.length && <p className="library-empty-copy">{zh ? '还没有专题，创建一个开始整理。' : 'Create your first topic to start organizing.'}</p>}</div><footer className="modal-footer"><button className="primary-button" onClick={() => { setTopic({ id: crypto.randomUUID(), name: '', documentIds: [] }); setError('') }}><PlusIcon />{zh ? '新建专题' : 'New topic'}</button></footer></LibraryDialog>}
