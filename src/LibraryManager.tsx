...
export function LibraryManager({ language, documents, selected, onOpen, onScope, onDiscuss, refresh, filters, selectionActions, onClear }: {
  ...
  return <>
    ...
    {trash && <LibraryDialog title={zh ? '回收站' : 'Trash'} busy={busy} onClose={() => setTrash(null)}><p className="modal-copy">{zh ? '这里管理资料库副本。来源能力的内部记录和历史引用快照会保留；永久删除后版本历史无法恢复。' : 'Manage Library copies here. Capability records and historical citation snapshots are retained. Permanent deletion also removes revision history.'}</p><div className="library-trash-list">{trash.map((doc) => <label key={doc.id}><input type="checkbox" disabled={busy} checked={trashIds.includes(doc.id)} onChange={() => setTrashIds((ids) => ids.includes(doc.id) ? ids.filter((id) => id !== doc.id) : [...ids, doc.id])} /><span><strong>{doc.title}</strong><small>{doc.capabilityName} · {doc.documentDate}</small></span></label>)}{!trash.length && <p>{zh ? '回收站是空的。' : 'Trash is empty.'}</p>}</div>{error && <p role="alert" className="library-error">{error}</p>}<footer className="modal-footer"><button className="secondary-button" disabled={busy || !trashIds.length} onClick={() => setConfirm('purge')}>{zh ? '永久删除' : 'Delete permanently'}</button><button className="primary-button" disabled={busy || !trashIds.length} onClick={() => void act(async () => { await changeLibrary({ kind: 'restore', ids: trashIds }); setTrash(await libraryTrash()); setTrashIds([]) })}>{zh ? '恢复' : 'Restore'}</button></footer></LibraryDialog>}
