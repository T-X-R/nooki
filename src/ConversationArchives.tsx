...
export function ConversationArchives({ language, onRestore }: { language: 'zh' | 'en'; onRestore(id: string): void }) {
  ...
  return <section className="settings-section settings-archive-section">
    <h2 className="conversation-archive-heading"><button className="conversation-archive-toggle" aria-label={zh ? '已归档会话' : 'Archived conversations'} aria-expanded={expanded} aria-controls="settings-conversation-archives" disabled={busy} onClick={() => { setExpanded(!expanded); setDeleting(null) }}><span className="settings-number">05</span><span>{zh ? '已归档会话' : 'Archived conversations'}</span><span className="conversation-archive-count" aria-live="polite" aria-label={loading ? (zh ? '正在读取归档数量' : 'Loading archive count') : loaded ? (zh ? `${sessions.length} 个已归档会话` : `${sessions.length} archived conversations`) : (zh ? '归档数量暂不可用' : 'Archive count unavailable')}>{loading ? '…' : loaded ? sessions.length : '—'}</span><ChevronRightIcon aria-hidden="true" /></button></h2>
    ...
    {deleting ? <>
      ...
      <footer className="archive-actions"><button className="quiet-button library-delete-action" disabled={busy || loading || !!error || !sessions.length} onClick={() => setDeleting(sessions)}><TrashIcon />{zh ? '清空归档' : 'Delete all archived'}</button></footer>
    </>}
