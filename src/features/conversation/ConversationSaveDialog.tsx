...
import { listLibraryDocuments, libraryOrganization, type LibraryDocumentMetadata, type Organization } from '../../platform/document-library.ts'
import { buildLibraryTree } from '../library/library-tree.ts'
import { CONVERSATION_OWNER, type SaveAnswerInput } from './conversation-model.ts'
...
export function ConversationSaveDialog({ input, onClose }: { input: SaveAnswerInput; onClose(): void }) {
  ...
  return <LibraryDialog className="conversation-save-dialog" title={zh ? '保存到资料库' : 'Save to Library'} busy={busy} onClose={onClose}>
    ...
    {library && <div className="conversation-save-destination">
      <DestinationSelect label={zh ? '专题' : 'Topic'} autoFocus value={topicId} disabled={busy} options={[{ id: '', name: zh ? '默认' : 'Default' }, ...library.organization.topics]} onChange={(id) => { setTopicId(id); setSectionId(CONVERSATION_OWNER) }} />
      <DestinationSelect label={zh ? '栏目' : 'Section'} value={section.id} disabled={busy} options={sections} onChange={setSectionId} />
    </div>}
