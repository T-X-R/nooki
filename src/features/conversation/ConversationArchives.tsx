...
import { ChevronRightIcon, ResetIcon, TrashIcon } from '@radix-ui/react-icons'
...
// Archives close the Settings page as its last section: putting a conversation away and clearing it
// out is local-data maintenance, and it stays folded so it never crowds the conversation itself.
export function ConversationArchives({ language, number, onRestore }: { language: 'zh' | 'en'; number: string; onRestore(id: string): void }) {
  ...
  return <section className="settings-section settings-archive-section">
    <div className="conversation-archive-header"><h2 className="conversation-archive-heading"><button className="conversation-archive-toggle" aria-label={zh ? '已归档会话' : 'Archived conversations'} aria-expanded={expanded} aria-controls="settings-conversation-archives" disabled={busy} onClick={() => { setExpanded(!expanded); setDeleting(null) }}><span className="settings-number">{number}</span><span>{zh ? '已归档会话' : 'Archived conversations'}</span><span aria-hidden="true" /><span className="conversation-archive-count" aria-live="polite" aria-label={loading ? (zh ? '正在读取归档数量' : 'Loading archive count') : loaded ? (zh ? `${sessions.length} 个已归档会话` : `${sessions.length} archived conversations`) : (zh ? '归档数量暂不可用' : 'Archive count unavailable')}>{loading ? '…' : loaded ? sessions.length : '—'}</span><ChevronRightIcon aria-hidden="true" /></button></h2><button className="quiet-button library-delete-action conversation-archive-clear" disabled={busy || loading || !!error || !sessions.length || !!deleting} onClick={() => { setExpanded(true); setDeleting(sessions) }}><TrashIcon />{zh ? '删除全部' : 'Delete all'}</button></div>
    {/* The rows say what they are. An intro line above them only spends the space they need. */}
