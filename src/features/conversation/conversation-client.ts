...
import { applyConversationEvent, retainPublicSummary, type Conversation, type ConversationEvent } from './conversation-model.ts'
...
function desktop() { if (!window.__TAURI_INTERNALS__) throw new Error('对话需要桌面版 Nooki 和已安装的 agent / Desktop Nooki and an installed agent are required') }
