...
import type { DocumentReference, InstalledCapability } from '../../../packages/capability-contract/src/index.ts'
import { activityStore } from '../../platform/activity.ts'
import { todayFeed } from '../../platform/activity-store.ts'
import { taskRunner } from '../../platform/tasks.ts'
import { CONVERSATION_OWNER } from '../conversation/conversation-model.ts'
...
export function TodayActivity({ language, installed, onDocument, onTask, onCapability }: {
  ...
  return <section className="activity-section">
    ...
    <span className="section-kicker">ACTIVITY</span><div className="section-heading-row activity-heading"><h2>{zh ? '最近活动' : 'Recent activity'}</h2><span className="activity-count">{feed.events.length}</span></div>
    {error && <p role="alert">{error}</p>}
    {!feed.events.length ? <div className="activity-empty">{zh ? '保存资料或生成文档后，活动会出现在这里。' : 'Your saved entries and generated documents will appear here.'}</div> : <div className="activity-list" role="region" aria-label={zh ? '最近活动' : 'Recent activity'} tabIndex={0}>{feed.events.slice(0, limit).map((event) => {
