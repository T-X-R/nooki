...
export type LibraryDocumentMetadata = Omit<DocumentPublication, 'content'> & {
  ...
  createdAt: string; updatedAt: string; revision?: string
}
...
export type Organization = { topics: Topic[]; trash: Record<string, string>; origins: Record<string, Origin> }
    ...
    change(change: LibraryChange): LibraryDocumentMetadata | null {
      ...
      switch (change.kind) {
        ...
        case 'purge':
          ...
          change.ids.forEach((id) => { delete state.history[id]; delete state.organization.trash[id]; delete state.organization.origins[id] })
