...
export function ConversationPage({ language, incomingIds, onConsumed, targetId, onTargetConsumed, onDocument }: { language: 'zh' | 'en'; incomingIds: string[]; onConsumed(): void; targetId: string | null; onTargetConsumed(): void; onDocument(ref: DocumentReference): void }) {
  ...
  return <div className="conversation-page">
    <header className="conversation-page-header"><div><span className="eyebrow">CONVERSATIONS</span><h1>{zh ? '对话' : 'Conversations'}</h1></div><span className="conversation-runtime"><span />Codex</span></header>
    <div className="conversation-layout">
      <aside className="conversation-sidebar"><button className="conversation-new" onClick={() => select(null)}><PlusIcon />{zh ? '新对话' : 'New conversation'}</button><div className="conversation-sidebar-label">{zh ? '最近对话' : 'Recent conversations'}</div>
        ...
        {!!legacy.length && <button className="quiet-button conversation-legacy-link" onClick={() => setShowLegacy(!showLegacy)}>{zh ? '以前的回顾草稿' : 'Previous review drafts'}</button>}
      </aside>
      <section className="conversation-main" aria-label={zh ? '当前对话' : 'Current conversation'}>
        <header className="conversation-thread-header"><strong>{thread?.name || thread?.preview || (zh ? '新的想法，从这里开始' : 'Start with a question')}</strong>{selected && <button className="icon-button" aria-label={zh ? '刷新对话' : 'Refresh conversation'} onClick={() => void act(() => conversationClient.read(selected))}><ReloadIcon /></button>}</header>
        <div className="conversation-messages" ref={scroll} onScroll={() => { const el = scroll.current!; follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 70 }}>
          {showLegacy ? <div className="conversation-legacy">{legacy.map((task) => { const draft = task.result as { id: string; title: string; content: string }; return <article key={task.id}><h3>{draft.title}</h3><div className="conversation-markdown"><ConversationMarkdown text={draft.content} onDocument={onDocument} /></div><button className="quiet-button" onClick={() => save(draft.id, draft.content, draft.title)}>{zh ? '保存到资料库' : 'Save to Library'}</button></article> })}</div> : !thread?.turns.length && !running ? <div className="conversation-empty"><span className="conversation-empty-icon"><ChatBubbleIcon /></span><h2>{zh ? '让资料成为下一步的起点' : 'Put your knowledge to work'}</h2><p>{zh ? '提一个问题，整理一个想法，或引用资料继续探索。' : 'Ask a question, shape an idea, or explore your Library.'}</p><div className="conversation-suggestions">{(zh ? ['从资料中提取关键信息', '对比几份资料的不同观点', '帮我整理下一步计划'] : ['Extract key information', 'Compare different perspectives', 'Help me plan my next steps']).map((example) => <button key={example} onClick={() => { setText(example); composer.current?.focus() }}>{example}<ArrowUpIcon /></button>)}</div></div> : <>
        ...
        <div className="conversation-composer-area">
          ...
          <form className="conversation-composer" onSubmit={(event) => { event.preventDefault(); send() }}>
            ...
            <textarea ref={composer} aria-label={zh ? '消息' : 'Message'} placeholder={zh ? '询问 Codex，或添加资料一起讨论…' : 'Ask Codex, or add documents to discuss…'} value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); if (!busy) send() } }} />
          ...
          </form><p className="conversation-composer-note">{zh ? '由本机 Codex 驱动 · 回复与历史保存在 Codex 会话中' : 'Powered by local Codex · Replies and history stay in Codex sessions'}</p>
        </div>
      </section>
    </div>
