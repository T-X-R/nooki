...
function WeeklyReviewPage({ host }: CapabilityPageProps) {
  ...
  const status = (record: TaskRecord) => {
    ...
    return <div className="weekly-status" aria-live="polite"><span>{labels[record.status]}{record.status === 'running' && record.stage ? ` · ${stages[record.stage] ?? record.stage}` : ''}<small>{record.error}</small></span>
  ...
  return <div className="weekly-page" lang={environment.locale} data-theme={environment.theme}>
    <header><span>WEEKLY REVIEW</span><h1>{zh ? '每周回顾' : 'Weekly Review'}</h1><p>{zh ? '从你选定的资料中回顾这一周。先生成草稿，确认后再发布。' : 'Reflect on the week using your selected documents. Generate a draft, then approve publication.'}</p></header>
    {error && <p role="alert">{error}</p>}
    <section className="weekly-scope"><h2>{zh ? '确认输入范围' : 'Confirm input scope'}</h2>
      {!grants.length ? <p>{zh ? '请先到资料库选择本周资料，并授权“每周回顾”读取。' : 'Select this week’s documents in the Library and authorize Weekly Review to read them.'}</p> : <>
        <select aria-label={zh ? '授权范围' : 'Authorized selection'} value={grantId} onChange={(event) => { setGrantId(event.target.value); setConfirmed(false) }}>
        ...
        <ul>{grant?.documents.map((doc) => <li key={doc.reference.documentId}><button onClick={() => host.documents.open(doc.reference)}>{doc.reference.title}</button><small>{doc.documentDate}</small></li>)}</ul>
        <label><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />{zh ? '确认仅使用以上资料快照，并发送给当前平台 AI Provider' : 'Use only these snapshots and send them to the selected platform AI Provider'}</label>
        ...
        })}>{zh ? '开始生成草稿' : 'Generate draft'}</button>
      </>}
    </section>
    {task && <section className="weekly-draft"><div className="weekly-draft-heading"><h2>{zh ? '回顾草稿' : 'Review draft'}</h2><select aria-label={zh ? '生成历史' : 'Generation history'} value={task.id} onChange={(event) => setDraftTaskId(event.target.value)}>{[...generations].reverse().map((item) => <option key={item.id} value={item.id}>{new Date(item.createdAt).toLocaleString(environment.locale)}</option>)}</select></div>
      {status(task)}
      {draft && <><p>{zh ? '本草稿使用的原文：' : 'Sources used for this draft:'} {draft.sources.map((reference) => <button key={reference.documentId} onClick={() => host.documents.open(reference)}>{reference.title}</button>)}</p><div className="weekly-output"><Markdown remarkPlugins={[remarkGfm]} components={{ a: ({ href, children }) => { const ref = parseReferenceHref(href ?? ''); return ref ? <a href={href} onClick={(event) => { event.preventDefault(); host.documents.open(ref) }}>{children}</a> : <a href={href}>{children}</a> } }}>{draft.content}</Markdown></div>
        {publication ? <>{status(publication)}{publication.status === 'completed' && <button onClick={() => host.documents.open(publication.result as DocumentReference)}>{zh ? '打开已发布文档' : 'Open published document'}</button>}</> : <div className="weekly-publish"><p>{zh ? '草稿尚未发布。请检查内容和来源引用后确认。' : 'This draft is unpublished. Check its content and source citations before confirming.'}</p><button className="weekly-primary" disabled={busy || tasks.some((item) => item.job === 'publish' && item.status === 'running')} onClick={() => void act(() => host.tasks.start('publish', { draft }))}>{zh ? '确认并发布到资料库' : 'Confirm and publish to Library'}</button></div>}
      </>}
    </section>}
  </div>
}
