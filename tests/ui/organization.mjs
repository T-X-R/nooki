...
try {
  ...
  // The conversation surface carries one conversation and its composer. Archives live in a dialog.
  assert.equal(await page.locator('.conversation-page .conversation-archive-list').count(), 0);
  ...
  const archiveEntry = page.locator('#sidebar-conversation-history').getByRole('button', { name: '已归档会话', exact: true });
  const archive = page.getByRole('dialog', { name: '已归档会话', exact: true });
  await archiveEntry.click();
  await archive.waitFor();
  assert.equal(await page.locator('.conversation-page .conversation-archive-list').count(), 0);
  await archive.getByRole('button', { name: '恢复会话：整理本周的想法', exact: true }).waitFor();
  await page.waitForFunction(() => document.querySelectorAll('.conversation-archive-row').length === 44);
  await archive.getByText('44 个已归档会话。恢复后可继续讨论，也可以永久删除。', { exact: true }).waitFor();
  assert.ok((await page.evaluate(() => window.qa.pages)).includes(40));
  await page.screenshot({ path: '/private/tmp/nooki-archive-dialog.png', animations: 'disabled' });
  ...
  const confirmation = archive.locator('.conversation-archive-confirm');
  await confirmation.waitFor();
  assert.equal(await page.evaluate(() => window.qa.deleted.length), 0);
  await page.screenshot({ path: '/private/tmp/nooki-archive-delete-confirmation.png', animations: 'disabled' });
  await confirmation.getByRole('button', { name: '取消', exact: true }).click();
  assert.equal(await page.evaluate(() => window.qa.deleted.length), 0);
  await archive.getByRole('button', { name: '删除会话：往期对话 1', exact: true }).click();
  await confirmation.getByRole('button', { name: '永久删除', exact: true }).click();
  await confirmation.waitFor({ state: 'hidden' });
  await page.waitForFunction(() => document.querySelectorAll('.conversation-archive-row').length === 42);
  ...
  await archive.getByRole('button', { name: '删除全部', exact: true }).click();
  await confirmation.waitFor();
  await page.keyboard.press('Escape');
  await archive.waitFor({ state: 'hidden' });
  assert.equal(await page.evaluate(() => window.qa.deleted.length), 2);
  ...
  await archive.locator('.modal-footer').getByRole('button', { name: '关闭', exact: true }).click();
  await archive.waitFor({ state: 'hidden' });
  ...
  const englishEntry = page.locator('#sidebar-conversation-history').getByRole('button', { name: 'Archived conversations', exact: true });
  const englishArchive = page.getByRole('dialog', { name: 'Archived conversations', exact: true });
  await englishEntry.focus(); await page.keyboard.press('Enter');
  ...
  assert.equal(await englishArchive.count(), 0);
  await page.evaluate(() => window.qa.failList = true);
  await page.getByRole('button', { name: 'Conversations', exact: true }).click();
  ...
  await englishArchive.getByText('0 archived conversations. Restore to continue, or delete permanently.', { exact: true }).waitFor();
  assert.deepEqual(errors, []);
  console.log('PASS: inline topics, empty scope, section persistence, duplicate validation, moving documents, theme/viewport QA, archive failure, restore from the conversation list, pagination, cancel, single deletion and partial bulk retry');
